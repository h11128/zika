...
class PerformanceContext:
    """Context manager for performance measurements compatible with tests."""
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        measure_performance(self.name, duration_ms, self.metadata)


def performance_context(name: str, metadata: Optional[Dict[str, Any]] = None) -> PerformanceContext:
    """Factory to create a performance measurement context used in tests."""
    return PerformanceContext(name, metadata)
...
# Backward-compatible cache tracker API used in tests
...
_cache_tracker: Optional[CachePerformanceTracker] = None

def get_cache_tracker() -> CachePerformanceTracker:
    global _cache_tracker
    if _cache_tracker is None:
        _cache_tracker = CachePerformanceTracker()
    return _cache_tracker

def record_cache_hit():
    get_cache_tracker().record_hit()

def record_cache_miss():
    get_cache_tracker().record_miss()

def get_cache_stats() -> Dict[str, int]:
    return get_cache_tracker().get_stats()

# Backward-compatible targets API used in tests
PERFORMANCE_TARGETS = {
    'first_render_ms': 500,
    'cached_render_ms': 100,
    'memory_usage_mb': 50,
    'cache_hit_rate_percent': 80,
}

def validate_performance_targets() -> Dict[str, bool]:
    # Provide simple validation based on current recorded metrics and cache stats
    monitor = get_performance_monitor()
    res: Dict[str, bool] = {}
    # Our tests mostly rely on cache hit rate
    stats = get_cache_stats()
    res['cache_hit_rate'] = stats.get('hit_rate', 0) >= PERFORMANCE_TARGETS['cache_hit_rate_percent']
    return res
