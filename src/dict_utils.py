...
# Module-level caches to avoid repeated heavy loads across instances/tests
_MINI_DICT_CACHE: Dict[str, Dict[str, List[str]]] = {}
_CEDICT_CACHE: Dict[str, Tuple[Dict[str, List[str]], int]] = {}
...
class ChineseDict:
    ...
    def __init__(self):
        ...
        self.loaded_sources = []
        # Lazy CEDICT loading support
        self._cedict_path: Optional[str] = None
        self._cedict_loaded: bool = False
    ...
    def load_mini_dict(self, path: str) -> bool:
        ...
        try:
            ...
            # Use module-level cache keyed by absolute path
            abs_path = str(Path(path).resolve())
            cached = _MINI_DICT_CACHE.get(abs_path)
            if cached is None:
                with open(abs_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                _MINI_DICT_CACHE[abs_path] = data
                cached = data
            # Share the cached mapping (read-only usage expected)
            self.mini_dict = cached
    ...
    def load_cedict_file(self, path: str) -> bool:
        ...
        try:
            ...
            abs_path = str(Path(path).resolve())
            cached = _CEDICT_CACHE.get(abs_path)
            if cached is None:
                cedict_map: Dict[str, List[str]] = {}
                entry_count = 0
                with open(abs_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        line = line.strip()
                        ...
                        for hanzi in (traditional, simplified):
                            if hanzi not in cedict_map:
                                cedict_map[hanzi] = def_list.copy()
                            else:
                                existing = set(cedict_map[hanzi])
                                for def_item in def_list:
                                    if def_item not in existing:
                                        cedict_map[hanzi].append(def_item)
                        entry_count += 1
                _CEDICT_CACHE[abs_path] = (cedict_map, entry_count)
                cached = _CEDICT_CACHE[abs_path]

            self.cedict_data, entry_count = cached
            self._cedict_loaded = True
    ...
    def _ensure_cedict_loaded(self) -> None:
        """Load CEDICT lazily on first use if a path is configured and not yet loaded."""
        if self._cedict_loaded:
            return
        if self._cedict_path and os.path.exists(self._cedict_path):
            # load via existing API to leverage caches
            self.load_cedict_file(self._cedict_path)
    ...
    def lookup_translation(self, word: str, max_definitions: int = 3) -> Optional[str]:
        ...
        # Then try CEDICT (lazy-load on demand)
        if not self.cedict_data:
            self._ensure_cedict_loaded()
    ...
    def get_all_translations(self, word: str, max_definitions: int = 3) -> Dict[str, Optional[str]]:
        ...
        # CEDICT with cleaning (lazy-load on demand)
        if not self.cedict_data:
            self._ensure_cedict_loaded()
        if word in self.cedict_data:
            definitions = self.cedict_data[word][:max_definitions]
...
def create_default_dict(data_dir: str = "data") -> ChineseDict:
    ...
    Uses module-level caches so multiple calls share parsed data and avoid
    repeated heavy allocations.

    Lazy-loads CEDICT: we only set the path here; actual parsing happens on
    first lookup that needs CEDICT (mini miss).
    """
    dict_instance = ChineseDict()

    # Load mini dictionary (shared)
    mini_path = os.path.join(data_dir, "mini_cedict.json")
    dict_instance.load_mini_dict(mini_path)

    # Configure CEDICT path for lazy loading (shared)
    cedict_path = os.path.join(data_dir, "cedict_ts.txt")
    if os.path.exists(cedict_path):
        dict_instance._cedict_path = cedict_path
    else:
        cedict_u8_path = os.path.join(data_dir, "cedict_ts.u8")
        if os.path.exists(cedict_u8_path):
            dict_instance._cedict_path = cedict_u8_path
