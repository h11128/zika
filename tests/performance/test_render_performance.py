...
class TestRenderPerformanceBaselines:
    ...
    def test_memory_usage_baseline(self):
        ...
        # Verify memory baseline using per-test delta (peak relative to start)
        peak_delta_mb = metrics['memory_peak_mb'] - metrics['memory_start_mb']
        assert peak_delta_mb < self.MEMORY_BASELINE_MB, \
            f"Peak memory delta {peak_delta_mb:.2f}MB exceeds baseline of {self.MEMORY_BASELINE_MB}MB (abs peak {metrics['memory_peak_mb']:.2f}MB, start {metrics['memory_start_mb']:.2f}MB)"
        ...
        # Isolated subprocess absolute check (clean interpreter): run the same loop and assert absolute peak
        import subprocess
        import textwrap
        code = textwrap.dedent(
            ...
            class PerformanceMonitor:
                ...
                def start_monitoring(self):
                    ...
                    self.peak_memory = self.start_memory
                def end_monitoring(self):
                    end = self.process.memory_info().rss/1024/1024
                    self.peak_memory = max(self.peak_memory, end)
            class MockRenderEngine:
                def __init__(self):
                    self.cache={}
                def _simulate_rendering_work(self, n):
                    time.sleep(0.001 + n*0.0001)
                def render_preview(self, cards_data, digest, use_cache=True):
                    if use_cache and digest in self.cache:
                        return self.cache[digest]
                    self._simulate_rendering_work(len(cards_data))
                    res=f"<div>Rendered {len(cards_data)} cards</div>"
                    if use_cache:
                        self.cache[digest]=res
                    return res
            # Prepare data
            cards_data=[{'uuid':f'card-{i}','hanzi':f'字{i}','pinyin':f'zì{i}','english':f'char{i}'} for i in range(1000)]
            engine=MockRenderEngine()
            mon=PerformanceMonitor()
            mon.start_monitoring()
            for i in range(10):
                engine.render_preview(cards_data, f'test_digest_memory_{i}', use_cache=True)
            mon.end_monitoring()
            print(f"{mon.peak_memory:.2f}")
            """
        )
        proc = subprocess.run([sys.executable, "-c", code], capture_output=True, text=True, check=True)
        abs_peak = float(proc.stdout.strip())
        assert abs_peak < 200, f"Isolated absolute peak {abs_peak:.2f}MB exceeds baseline of 200MB"
